...
app.use(cors({ origin: 'http://localhost:5173', credentials: true })); // Default Vite port, allow cookies
...
app.get('/api/seed-admin-global', async (req: Request, res: Response) => {
  try {
    ...
    await User.create({ name: 'Admin Garima', email: adminEmail, password: hashedPassword, role: 'admin' });
