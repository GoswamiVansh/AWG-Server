...
const generateSlug = (name) => name.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + Math.floor(Math.random() * 1000);
const productsInput = [
    // Bouquets
...
const seedProducts = async () => {
    try {
        ...
        for (const cat of categories) {
            let doc = await Category_1.default.findOne({ slug: cat.slug });
            if (!doc) {
                doc = await Category_1.default.create(cat);
            }
        ...
        for (const prodInput of productsInput) {
            ...
            await Product_1.default.create(newProduct);
        }
